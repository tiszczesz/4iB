...
FROM `uczestnicy` INNER JOIN miejsca on uczestnicy.miejsce_id=miejsca.id GROUP by month(miejsca.data_wyjazdu) 
HAVING COUNT(uczestnicy.id)>=5 ORDER BY miejsca.nazwa